...
class IOPipeMemoryTest extends AbstractProlTest {
  ...
  void testIOMemory() throws Exception {
    ...
    final ProlConsult consult = new ProlConsult(new StringReader("save_to_memory([]):-!.save_to_memory([X|L]):-write(X),write(\'. \'),save_to_memory(L). all_from_memory :- next_from_memory(X),write(X),nl,(all_from_memory,!;true). next_from_memory(X):-read(X),X\\==end_of_file."), context);
    consult.consult();
