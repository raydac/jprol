...
public class PrimitiveTest extends TestCase {
  ...
  public void testPermutation() throws Exception {
    ...
    for (int li = 0; li < etalon.length; li++) {
      assertNotNull(goal.solve());
      assertEquals(goal.getVarAsText("X"), etalon[li]);
    }
    assertNull(goal.solve());
  }
  ...
  public void testOr1() throws Exception {
    ...
    for (int li = 0; li < results.length; li++) {
      assertNotNull(goal.solve());
      assertEquals(goal.getVarAsText("X"), results[li]);
    }
  ...
  public void testORGoal() throws Exception {
    ...
    for (int la = 0; la < results.length; la++) {
      assertNotNull(goal.solve());
      assertEquals(goal.getVarAsText("A"), results[la]);
    }
    assertNull(goal.solve());
  }
  ...
  public void testCutAnd() throws Exception {
    ...
    for (int la = 0; la < resultsA.length; la++) {
      for (int lb = 0; lb < resultsB.length; lb++) {
        assertNotNull(goal.solve());
        assertEquals(goal.getVarAsText("A"), resultsA[la]);
        assertEquals(goal.getVarAsText("B"), resultsB[lb]);
      }
