...
public final class ProlContext {
    ...
    public void halt() {
        ...
        try {
            try {
                for (ProlMemoryPipe prolMemoryPipe : pipes.values()) {
                    try {
                        prolMemoryPipe.close();
                    } catch (Throwable thr) {
                    }
                }

                for (ProlTextInputStream prolTextInputStream : inputStreams.values()) {
                    try {
                        prolTextInputStream.close();
                    } catch (Throwable thr) {
                    }
                }
                inputStreams.clear();

                for (ProlTextOutputStream prolTextOutputStream : outputStreams.values()) {
                    try {
                        prolTextOutputStream.close();
                    } catch (Throwable thr) {
                    }
                }
                outputStreams.clear();
