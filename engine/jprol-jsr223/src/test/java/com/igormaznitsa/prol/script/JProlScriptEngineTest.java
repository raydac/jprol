...
class JProlScriptEngineTest {
  ...
  @Test
  void testCompiled() throws Exception {
    final ScriptEngine engine = JPROL_ENGINE_FACTORY.getScriptEngine();
  ...
  @Test
  void testBindings() throws Exception {
    final ScriptEngine engine = JPROL_ENGINE_FACTORY.getScriptEngine();
    engine.put("X", "world");
  ...
  @Test
  void testBindAndGet() throws Exception {
    final ScriptEngine engine = JPROL_ENGINE_FACTORY.getScriptEngine();
    engine.put("X", 5);
  ...
  void testAssert() throws Exception {
    ...
    assertEquals("some(1)", found.get(0).toSrcString());
    assertEquals("some(2)", found.get(1).toSrcString());
    assertEquals("some(3)", found.get(2).toSrcString());
